...
/**
 * @brief テキストエレメントをトークン化する。そのテキストの中に合致する正規表現があればさらにその合致したテキストをトークン化する
 ...
 */
static int __tokenizeText(char *textElement[], int initial_id, Token_t *initial_root);
...
static List_T __tokenizeText(char *text_element[], int initial_id, Token_t *initial_root)
{
